  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (imageFile) {
        ...
        imageUrl = await uploadImage(imageFormData) as string;
      ...
      // Update blog
      const result = await updateBlog(blog.id, formData, imageUrl);
            ...
            <div className="space-y-4 flex flex-col h-full">
              <div>
                <input
                  ...
                  autoFocus
                  required
                  className="w-full px-0 py-2 border-b-2 border-brand-border/60 focus:border-brand-gold outline-none transition-all text-xl sm:text-2xl font-playfair font-bold text-brand-charcoal bg-transparent"
        ...
        <div className="w-full lg:w-80 xl:w-96 flex-shrink-0 flex flex-col gap-4 overflow-y-auto pr-2 pb-4">
          ...
          <div className="hidden lg:flex bg-white rounded-xl shadow-sm border border-brand-border/60 p-4 justify-between items-center">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex items-center gap-1.5 px-5 py-2 rounded-lg text-xs font-semibold text-white bg-blue-600 hover:bg-blue-700 transition-colors disabled:opacity-70 shadow-sm"
            >
              {loading ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />}
