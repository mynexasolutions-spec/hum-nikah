...
import { uploadGalleryImage, createGalleryItem } from "../actions";
...
export default function CreateGalleryItemPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const imageUrl = await uploadGalleryImage(imageFormData) as string;

      if (!imageUrl) {
        throw new Error("Failed to upload image");
      }

      const result = await createGalleryItem(formData, imageUrl);
  ...
  return (
    <div className="p-4 sm:p-6 max-w-[1200px] mx-auto w-full h-[calc(100vh-theme(spacing.20))] flex flex-col">
      <div className="mb-4 flex items-center justify-between gap-4 flex-shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-brand-charcoal font-playfair leading-tight">Upload Photo</h1>
            <p className="text-xs text-brand-secondary">Add a new photo to your gallery.</p>
          </div>
        </div>
        ...
        <button
          ...
          type="submit"
          disabled={loading}
          className="lg:hidden flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-semibold text-white bg-[#062E29] hover:bg-[#062E29]/90 transition-colors disabled:opacity-70 shadow-sm"
        >
      ...
      <form id="gallery-form" onSubmit={handleSubmit} className="flex-grow flex flex-col lg:flex-row gap-6 min-h-0">
        {/* Left Side: Image Upload (Reduced Width) */}
        <div className="w-full lg:w-1/3 flex-shrink-0 flex flex-col gap-4 overflow-y-auto pr-2 pb-4">
          <div className="bg-white rounded-xl shadow-sm border border-brand-border/60 p-4">
            ...
            <div 
              className={`w-full aspect-square border-2 border-dashed rounded-xl overflow-hidden relative flex items-center justify-center transition-all ${
        ...
        <div className="flex-grow flex flex-col gap-4 overflow-visible pr-2 pb-4">
            ...
            <div className="space-y-6 flex flex-col h-full">
              <div>
                <input
                  ...
                  autoFocus
                  required
                  className="w-full px-0 py-2 border-b-2 border-brand-border/60 focus:border-[#b3854d] outline-none transition-all text-xl sm:text-2xl font-playfair font-bold placeholder:text-slate-300 bg-transparent"
